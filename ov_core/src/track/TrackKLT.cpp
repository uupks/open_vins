...
void TrackKLT::feed_monocular(const CameraData &message, size_t msg_id) {
  ...
  PRINT_ALL("[TIME-KLT]: %.4f seconds for feature DB update (%d features)\n", std::chrono::duration<double, std::micro>(rT5 - rT4).count()  * 1e-6,
            (int)good_left.size());
  PRINT_ALL("[TIME-KLT]: %.4f seconds for total\n", std::chrono::duration<double, std::micro>(rT5 - rT1).count()  * 1e-6);
}
...
void TrackKLT::feed_stereo(const CameraData &message, size_t msg_id_left, size_t msg_id_right) {
  ...
  PRINT_ALL("[TIME-KLT]: %.4f seconds for feature DB update (%d features)\n", std::chrono::duration<double, std::micro>(rT6 - rT5).count()  * 1e-6,
            (int)good_left.size());
  PRINT_ALL("[TIME-KLT]: %.4f seconds for total\n", std::chrono::duration<double, std::micro>(rT6 - rT1).count()  * 1e-6);
}
...
void TrackKLT::perform_matching(const std::vector<cv::Mat> &img0pyr, const std::vector<cv::Mat> &img1pyr, std::vector<cv::KeyPoint> &kpts0,
  ...
  // std::vector<cv::UMat> u_img0pyr, u_img1pyr;
  // for (size_t i = 0; i < img0pyr.size(); i++) {
  //   cv::UMat umat0; img0pyr.at(i).copyTo(umat0); 
  //   cv::UMat umat1; img1pyr.at(i).copyTo(umat1);
  //   u_img0pyr.push_back(umat0);
  //   u_img1pyr.push_back(umat1);
  // }
